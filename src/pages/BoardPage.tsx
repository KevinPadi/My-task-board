import { Check, CircleCheckBig, CircleFadingArrowUp, CirclePlus, CircleX, Loader, Trash2, X } from "lucide-react"
...
function AddNewTaskDrawer({ task }: { task: TaskSchema }) {
    ...
    <section>
      ...
      <div className="mt-20 max-w-2xl mx-auto p-4">
        <ul className="flex flex-col gap-4">
          {
            tasks.map((task) => (
              <AddNewTaskDrawer key={task._id} task={task} />
            ))
          }
        ...
        {/* Add task */}
        <button onClick={() => createTask()} className="w-full rounded-2xl bg-yellow-100/80 p-4 mt-4 hover:cursor-pointer hover:bg-yellow-200 transition-colors ease-in-out duration-300">
          <div className="flex w-full items-center justify-between">
            <div className="flex items-center gap-4">
